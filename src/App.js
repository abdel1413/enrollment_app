    ...
    const  getCoursesData = async ()=>{
     ...
     const response = await axios.get("http://localhost:3001/courselist");
      ...
      const displayCourses = async ()=>{
        await axios.get('http://localhost:3001/advancedcourses')
             .then((response)=>{
...
const [isChecked, setIsChecked]= useState(new Array(advCourse.length).fill(false))


// Keep track of selected items
const total = isChecked.filter(item => item).length + checkState.filter(item => item).length
//setTotalItems(total)
